...
namespace LinShinOR.MaterialsForm.Entity
{
    public class MaterialRecord : EntityBase, IEntityDrivenBase<MaterialRecord>
    {
        ...
        public static bool TryParse(List<string> GroupDataList, out MaterialRecord materialRecord)
        {
            ...
            int lineIndex = 0;
            foreach (KeyValuePair<string, Dictionary<string, int>> lineMap in MaterialIndexMap.LineMaps)
            {
        ...
        public bool IsValid()
        {
            if (string.IsNullOrWhiteSpace(this.AssetName)) return false;
            if (this.AssetName.Contains("=====") || this.AssetName.Contains("-----")) return false;

            return !string.IsNullOrWhiteSpace(this.StorageLocation)
                && !string.IsNullOrWhiteSpace(this.Category)
                && !string.IsNullOrWhiteSpace(this.AcquisitionDate)
                && !string.IsNullOrWhiteSpace(this.Quantity);
        }
